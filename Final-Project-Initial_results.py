...
def undersampling(dataset):
    ...
    dataset = pd.concat([final_fake_data, final_true_data], axis=0)
    

    #fig, ax = plt.subplots()
    #fig.suptitle("Dataset Balance", fontsize = 20)
...
def oversampling(dataset):
    ...
    print("data has been oversampled")
    return dataset
...
def apply_tfidf():
    ...
    #fit the 
    tfidf_vectorizer = tfidf_vectorizer.fit(dataset.text)
...
largest_test_score = 0.000000000000000
smallest_test_score = 0.000000000000000
...
for y_train, y_test in folds.split(dataset.text, dataset.label):
    ...
    #x_train, y_train = apply_cc(x_train, y_train)

    
    fig, ax = plt.subplots()
    fig.suptitle("Dataset Balance", fontsize = 20)
    y_train.reset_index().groupby('label').count().sort_values(by="index").plot(kind='barh', legend=False, ax=ax).grid(axis='x')
    plt.show()  
    ...
    #Below is code for storing the total score in order to calualte the average score over multiple folds of cross validation
    total_train_score = total_train_score + train_score
    total_test_score  = total_test_score  + test_score

    if (largest_test_score < test_score):
        largest_test_score = test_score
    if(count == 0):
        smallest_test_score = test_score
    if(test_score < smallest_test_score):
        smallest_test_score = test_score

    print("Time taken: ", time()-fold_time, "seconds")    
    print()
    print("-----------------------------------------")
    print()
...
print("-----------------------------------------")
print()
print("Largest score", largest_test_score)
print("Smallest score", smallest_test_score)
print("Standard deviation: ", (largest_test_score-smallest_test_score) )
print()
print("-----------------------------------------")
print()
...
print("The overall time of classification is:", time()-overall_time, "seconds")